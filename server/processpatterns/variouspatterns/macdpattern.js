...
const checkBuyMACDPattern = async (tp,stock,inpDate,dur,period,range) =>{
...
const mainPattern = async (pattern,stock,inpDate,storefunction) =>{
    ...
    if (pattern && pattern.params.length > 0){
        if (pattern.params[0]["BULLISH"]){
            await checkBuyMACDPattern(pattern["type"],stock,inpDate,pattern.params[0]["duration"],pattern.params[0]["period"],pattern.params[0]["BULLISH"]
